    ...
    try:
        ...
        def get(keys: str|list[str], default=''):
            for key in (keys,) if isinstance(keys, str) else keys:
                if (val := ronde.get(key, create.get(key) if create != ronde
                    else None)) is not None: return val
            return default
        ...
        for ronde in rounds:
            ...
            for pair in pairings:
                ...
                data = {
    "titre":        title, # str(.) ## we search for "placeholder=Titre" and fill in data['titre']
    # 'description' is filled into TEXTAREA :
    "description":  get(["desc", 'description'], "Rencontre de la compétition "+comp.name), #TODO : add "cut-off ..."
    "club_hote_id":     team1.club_id,
    "club_invite_name": team2.raw_data.get('name', team2.name), # this is more complicated to set, in the form
    # the date must be chosen in a datepicker:
    "date":             get(["date","start",'start_date'], comp.start_date), # format: DD/MM/YYYY
    "days_per_move":    get('days_per_move', "3"),
    ## this and similar : setVal(document.querySelector('input[name="min-players"]'), match.min_players)
